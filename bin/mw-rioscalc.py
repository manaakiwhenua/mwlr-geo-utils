...
parser.add_arugment('--calc', type=str, default="np.nanmean(rasters, axis=0)", help="Formula to apply, raster(s) available np array 'rasters', numpy is available as 'np'")
...
def apply(info, ins, outs, others):
    outs.result = eval(others.formula, {"rasters": ins.rasters})
